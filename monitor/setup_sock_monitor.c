...
void setup_sock_monitor_init()
{
    ...
    if (socket_fd == -1)
        FATAL("Cannot create socket for monitor, err:",strerror(errno));
    ...
    if (bind(socket_fd, (struct sockaddr *)&local, len) == -1)
        FATAL("Failed to bind for monitor, err:", strerror(errno));
    if (listen(socket_fd, 10000) == -1)
        FATAL("Failed to listen for monitor, err:", strerror(errno));
...
int setup_sock_accept(ctl_struc *result)
{
    ...
    if ((peer_fd = accept(socket_fd, NULL, NULL))==-1)
    {
        ...
        else FATAL("Failed to accept a new attach request");
    }
    int len_recvd=0;
    void* pointer=result;
